...
class PatriciaTrieNaive {
    ...
    struct LeafNode : public Node {
        LeafNode(len_t len, const char* str, node_pos_t ext_pos)
...
public:
    void Insert(std::string_view str, node_pos_t ext_node_pos) {
        ...
        Node* new_leaf = new LeafNode{(len_t)str.size(), str.data(), ext_node_pos};
...
private:
    ...
    std::tuple<InnerNode*, len_t, char_t> SearchInsertNode(std::string_view str) {
        Node* node = m_root;
        while (true) {
            assert(node->IsInner());
            ...
            if (it == branchs.end()) {
                ...
                len_t min_len = std::min((len_t)str.size(), child->len);
                len_t lcp = inn_node->len;
                for (; lcp < min_len; ++lcp) {
    ...
    // node is inner and must have child
    ...
    void InsertInNonRoot(Node* child, InnerNode* parent, std::string_view str,
                         node_pos_t ext_node_pos, len_t lcp) {}

private:
    InnerNode* m_root = new InnerNode{0};
};

void Draw(const PatriciaTrieNaive& pt, std::string file_name) {
    std::ofstream{file_name} << pt.DrawTrie();
}
